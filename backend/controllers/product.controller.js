...
export const getRecommendedProducts = async (req, res) => {
  try {
    const products = await Product.aggregate([
      ...
      { $project: {
		_id: 1, // Include only the necessary fields
		name: 1,
		description: 1,
		image: 1,
		price: 1,
...
export const getProductsByCategory = async (req, res) => {
	...
	try {
		const products = await Product.find({category}); // Find products by category from the database
		res.json({ products }); // Send the products as a JSON response
	} catch (error) {
...
async function updateFeaturedProductsCache() {
  try {
	const featuredProducts = await Product.find({ isFeatured: true }).lean(); // Fetch featured products from the database using .lean() for better performance and convert them to plain JavaScript objects
	
	await redis.set("featured_products", JSON.stringify(featuredProducts)); // Update the featured products cache in Redis
  } catch (error) {
